...
Dla każdej z tych zmiennych stworzymy histogram prawdopodobieństwa za pomocą funkcji \texttt{$geom_histogram$} i dopasujemy do niego jądrowy estymator gęstości używając \texttt{$geom_density$}. Wynik dla średnich ocen prezentuje się następująco
...
```
```{r}
# Zakładamy, że `rym` to Twoja ramka danych
filtered_data <- rym %>%
