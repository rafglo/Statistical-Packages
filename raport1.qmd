...
ggplot(genre_summary_df, aes(x = reorder(Genre, -Count))) +
  ...
  geom_line(aes(y = Count / max(Count) * max(Average_Rating), group = 1), color = "red", linewidth = 1.2) +  # Linia liczby wystąpień
  scale_y_continuous(
    name = "Średnia ocena",
...
ggplot(daxpp, aes(x = `Number of Ratings`, y = as.numeric(`Average Rating`))) +
  ...
  geom_smooth(formula = y ~ x, method = "lm", color = "red", se = TRUE) +  # Trend line (linear regression)
